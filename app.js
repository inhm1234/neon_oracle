...
const oracleBasisInput = document.getElementById("oracleBasisInput");
const oracleBasisPersonal = document.getElementById("oracleBasisPersonal");
const oracleBasisToday = document.getElementById("oracleBasisToday");
const oracleBasisSync = document.getElementById("oracleBasisSync");
const oracleBasisReason = document.getElementById("oracleBasisReason");
const oracleBasisFormula = document.getElementById("oracleBasisFormula");
...
const shareCardFrame = document.getElementById("shareCardFrame");
...
const DEV_VERSION = "V3-16.5 test";
...
function updateAdminToolsVisibility() {
  ...
  // V3-12: 관리자 계정으로 로그인되어 있어도 일반 주소에서는 관리자 도구를 숨깁니다.
...
function renderAdminStatsGate(message = "관리자 Google 계정으로 로그인하면 통계 대시보드를 볼 수 있습니다.") {
  ...
  if (adminStatsCard) {
    // V3-12: 통계 카드는 관리자 설정 주소(?admin=1)에서만 보입니다.
...
function formatElementScore(score) {
  return Object.entries(score)
    .map(([key, value]) => `${elementName[key]} ${Math.round(value * 10)}`)
    .join(" · ");
}

function buildOracleBasis({ profile, timeInfo, todayInfo, todayElement, elementScore, rankedElements, mainElement, subElement, relationReport, elementProfile, subProfile, pattern, daySignal }) {
  const genderText = getGenderLabel(profile.gender);
  const timeText = timeInfo ? `${timeInfo.name}시 ${timeInfo.range}` : "태어난 시간 모름";
  const scoreText = formatElementScore(elementScore);
  const mainText = elementName[mainElement];
  const subText = elementName[subElement];
...
const SHARE_CARD_DEFAULT_POSITIONS = [
  { x: 16, y: 18 },
  { x: 82, y: 20 },
  { x: 78, y: 42 },
  { x: 18, y: 66 },
...
function getShareThemeMeta(theme) {
  const meta = {
    lovely: { subnote: "말랑하고 사랑스러운 오늘의 기분", footer: "오늘의 기분을 귀엽게 저장해두는 카드", signature: "save me ♡" },
    sky: { subnote: "맑고 차분한 흐름을 담은 카드", footer: "차분한 오늘을 포근하게 남기는 카드", signature: "soft sky ♡" },
    twinkle: { subnote: "반짝이는 무드와 키워드를 담은 카드", footer: "반짝이는 순간을 저장하는 오늘의 카드", signature: "twinkle day ✦" }
  };
...
function renderShareCardStickers() {
  ...
  shareCardStickers.innerHTML = shareCardState.stickers.map((item, index) => {
    ...
    return `<button class="share-card-sticker" type="button" data-sticker-index="${index}" style="left:${x}%; top:${y}%; transform: translate(-50%, -50%) rotate(${rotate}deg)" aria-label="스티커 ${item.symbol} 이동하기">${item.symbol}</button>`;
  }).join("");
  bindShareStickerDragEvents();
}

function bindShareStickerDragEvents() {
  if (!shareCardStickers) return;
  const stickerEls = shareCardStickers.querySelectorAll(".share-card-sticker");
  stickerEls.forEach((stickerEl) => {
    stickerEl.addEventListener("pointerdown", startShareStickerDrag);
    stickerEl.addEventListener("dblclick", removeShareStickerByEvent);
  });
}
...
function startShareStickerDrag(event) {
  ...
  activeStickerDrag = { index, target };
  target.setPointerCapture(event.pointerId);
  target.classList.add("is-dragging");
  updateDraggedStickerPosition(event);
  target.addEventListener("pointermove", updateDraggedStickerPosition);
  target.addEventListener("pointerup", endShareStickerDrag);
  target.addEventListener("pointercancel", endShareStickerDrag);
}

function updateDraggedStickerPosition(event) {
  if (!activeStickerDrag || !shareCardPreview) return;
  const rect = shareCardPreview.getBoundingClientRect();
  ...
  const sticker = shareCardState.stickers[activeStickerDrag.index];
  if (!sticker) return;
  sticker.x = Math.round(x * 10) / 10;
  sticker.y = Math.round(y * 10) / 10;
  activeStickerDrag.target.style.left = `${sticker.x}%`;
  activeStickerDrag.target.style.top = `${sticker.y}%`;
}

function endShareStickerDrag(event) {
  if (!activeStickerDrag) return;
  const target = activeStickerDrag.target;
  target.classList.remove("is-dragging");
  target.removeEventListener("pointermove", updateDraggedStickerPosition);
  target.removeEventListener("pointerup", endShareStickerDrag);
  target.removeEventListener("pointercancel", endShareStickerDrag);
  activeStickerDrag = null;
  if (shareStudioStatus) shareStudioStatus.textContent = "스티커 위치를 옮겼어요.";
}

function removeShareStickerByEvent(event) {
  const index = Number(event.currentTarget.dataset.stickerIndex);
  if (!Number.isFinite(index)) return;
  shareCardState.stickers.splice(index, 1);
  renderShareCardPreview();
  if (shareStudioStatus) shareStudioStatus.textContent = "스티커 하나를 지웠어요.";
}
...
function renderShareCardPreview(result = latestShareResult) {
  ...
  if (shareCardDate) shareCardDate.textContent = `${formatTodayCardDate()} · share card`;
  ...
  if (shareCardSignature) shareCardSignature.textContent = themeMeta.signature;
...
function addStickerToShareCard(symbol) {
  ...
  const defaultPosition = SHARE_CARD_DEFAULT_POSITIONS[shareCardState.stickers.length % SHARE_CARD_DEFAULT_POSITIONS.length];
  shareCardState.stickers.push({
    symbol,
    x: defaultPosition.x,
    y: defaultPosition.y,
    rotate: (shareCardState.stickers.length % 2 === 0 ? -6 : 6)
  });
  renderShareCardPreview();
  if (shareStudioStatus) shareStudioStatus.textContent = `스티커 ${symbol} 를 붙였어요. 카드 위에서 직접 끌어서 원하는 위치에 놓을 수 있어요.`;
}
...
function splitSvgLines(value, maxChars = 16, maxLines = 3) {
  ...
  Array.from(text).forEach((char) => {
    current += char;
    if (current.length >= maxChars && chunks.length < maxLines - 1) {
...
function getExportThemeColors(theme) {
  const themes = {
    lovely: { pageA: "#fff6fb", pageB: "#f5f0ff", cardA: "#ffd7ea", cardB: "#fff7fb", cardC: "#f8efff", ink: "#6f4e80", soft: "#7d678c", accent: "#ffbfde", deco: "#ffd2e8" },
    sky: { pageA: "#f3fbff", pageB: "#eff2ff", cardA: "#d9f1ff", cardB: "#f9fcff", cardC: "#e7ecff", ink: "#536281", soft: "#66708e", accent: "#b9dcff", deco: "#cfeaff" },
    twinkle: { pageA: "#fff9ea", pageB: "#ffeef6", cardA: "#fff0ba", cardB: "#fff9ef", cardC: "#ffdcec", ink: "#7a586e", soft: "#806777", accent: "#ffd976", deco: "#ffe39d" }
  };
  return themes[theme] || themes.lovely;
}

function buildShareExportStage(format = "square") {
  const stage = document.createElement("div");
  stage.className = `share-export-stage ${format === "story" ? "story" : "square"}`;

  const shell = document.createElement("div");
  shell.className = "share-export-card-shell";

  const clone = shareCardPreview.cloneNode(true);
  clone.removeAttribute("id");
  const cloneStickers = clone.querySelectorAll('.share-card-sticker');
  cloneStickers.forEach((sticker) => {
    sticker.removeAttribute('data-sticker-index');
...
async function downloadShareCardImage(format = "square") {
  ...
  if (!window.html2canvas) {
    if (shareStudioStatus) shareStudioStatus.textContent = "저장 도구를 불러오지 못했어요. 새로고침 후 다시 시도해주세요.";
    return;
  }

  if (shareStudioStatus) shareStudioStatus.textContent = "보이는 카드를 그대로 저장하는 중이에요...";
  const stage = buildShareExportStage(format);
  ...
  try {
    ...
    if (shareStudioStatus) shareStudioStatus.textContent = "저장 중 문제가 생겼어요. 새로고침 후 다시 시도해주세요.";
  } finally {
    stage.remove();
  }
}
...
function renderResult(result, partnerReaction = null) {
  ...
  if (result.basis) {
    if (oracleBasisInput) oracleBasisInput.textContent = result.basis.input;
    if (oracleBasisPersonal) oracleBasisPersonal.textContent = result.basis.personal;
    if (oracleBasisToday) oracleBasisToday.textContent = result.basis.today;
    if (oracleBasisSync) oracleBasisSync.textContent = result.basis.sync;
    if (oracleBasisReason) oracleBasisReason.textContent = result.basis.reason;
    if (oracleBasisFormula) oracleBasisFormula.textContent = result.basis.formula;
  }
